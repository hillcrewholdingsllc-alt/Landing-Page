...
async function submit(extra={}){const payload={firstName:'Offline',phone:'2025550123',propertyAddress:'123 Fixture Street',formStartedAt:new Date(Date.now()-10000).toISOString(),...extra};const res=await worker.fetch(new Request('https://fixture/lead',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)}),{DB});return new URL(res.headers.get('location'));}
async function claim(dest,origin='https://kbuyhouses.com'){const p=new URLSearchParams(dest.hash.slice(1));return worker.fetch(new Request('https://fixture/conversion-receipt',{method:'POST',headers:{origin,'content-type':'application/json'},body:JSON.stringify({submissionId:p.get('submission'),receipt:p.get('receipt')})}),{DB});}
...
for(const path of ['/next','/ack','/fail']){const method=path==='/next'?'GET':'POST';const r=await worker.fetch(new Request('https://fixture'+path,{method,headers:{'cf-connecting-ip':'192.0.2.1'},...(method==='POST'?{body:'{}'}:{})}),{DB});assert.equal(r.status,403);count++;}
